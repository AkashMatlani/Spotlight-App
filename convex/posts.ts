...
//mutation is user of update data
        ...
        const postWithInfo = await Promise.all(posts.map(async (post) => {
            ...
            const like = await ctx.db.query("likes")
                .withIndex("by_user_and_post",
                    (q) => q.eq("userId",
                        currentUser._id).eq("postId", post._id))
                .first();
...
async function updateFollowCount(
    ctx: MutationCtx,
    followerId: Id<"users">,
    followingId: Id<"users">, isFollow: boolean) {
